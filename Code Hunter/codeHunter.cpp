...
int main()
{
	...
	//function that analyze each 
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		//looks to see if there are any vowels
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			//fixed the vowles from negative to positive
			++vowels;
		}
		// looks to see what what is a conssonant
		else if ((textToAnalyze[i] >= 'a' && textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A' && textToAnalyze[i] <= 'Z'))
		{
			//uncommented for consonants to work
			++consonants;
		}
		//looks to see if there are any digits
		else if (textToAnalyze[i] >= '0' && textToAnalyze[i] <= '9')
		{
			++digits;
		}
		//looks to see if there are any spaces
	...
	//function to check everything and state if it is working or not 
